define([ "geoladris-tests" ], function(tests) {
	...
	describe("ui-search", function() {
		...
		beforeEach(function(done) {
			...
			injector.require([ "ui-search" ], function() {
				done();
			});
		...
		it("creates a dialog on ui-search-results:create", function() {
			...
			expect(bus.send).toHaveBeenCalledWith("ui-dialog:create", jasmine.objectContaining({
				div : "mysearchresults",
				parentDiv : parentId,
				title : "Results",
				closeButton : true,
		...
		it("empties list on ui-search-results:clear", function() {
			...
			bus.send("ui-search-results:mysearchresults:clear");
		...
		it("adds item on ui-search-results:add", function() {
			...
			bus.send("ui-search-results:mysearchresults:add", "Result 1");
		...
		it("sends event on return key pressed on box", function() {
			...
			expect(bus.send).toHaveBeenCalledWith("ui-search-box:mysearchbox:search", value);
		});
		...
		it("sends event on list item clicked", function() {
			...
			expect(bus.send).toHaveBeenCalledWith("ui-search-results:mysearchresults:selected", "Result 1");
		});
	});
});